...
async function commonBeforeAll(){
  ...
  console.log(startDateStr)
    ...
    let managerIdQuery = await db.query(`SELECT * FROM positions WHERE position_name = 'Manager'`);
    let MANAGER_ID = managerIdQuery.rows[0].position_id;
    let certificationIdQuery = await db.query(`SELECT * FROM certifications where certification_name = 'None'`)
    let CERTIFICATION_ID = certificationIdQuery.rows[0].certification_id;
    ...
    VALUES($1,'Shawn','Rostas', 'matthewchanway@gmail.com',${MANAGER_ID}, ${CERTIFICATION_ID},$2,null,null,null,true)`, [p1Hash, initStr])
